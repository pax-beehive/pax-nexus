...
EXTRACTION_CANDIDATE_STRATEGIES := interaction-slim source-clause-v1 source-span-v1 source-span-v2 claim-card-v2
...
build-saas: validate-extraction-candidate-strategy validate-recall-candidate-strategy
	mkdir -p $(OUTPUT_BIN_DIR)
	CGO_ENABLED=0 GOCACHE=$${GOCACHE:-/tmp/team-memory-go-cache} go build -trimpath \
		-ldflags "$(EXTRACTION_CANDIDATE_LDFLAG) $(RECALL_CANDIDATE_LDFLAG)" -o $(OUTPUT_BIN_DIR)/hertz_service_saas ./cmd/team-memory-saas
...
integration-test: db-up
	TEAM_MEMORY_TEST_POSTGRES_DSN='$(TEAM_MEMORY_TEST_POSTGRES_DSN)' \
		GOCACHE=$${GOCACHE:-/tmp/team-memory-go-cache} go test -p 1 ./internal/platform/postgres ./internal/teamnote/extractionqueue ./internal/teamnote/transport/httpapi/handler ./internal/eval/stagecapture ./internal/eval/v2/postgresstore ./internal/app -count=1
