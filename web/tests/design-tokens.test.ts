...
describe("design tokens", () => {
  ...
  // 过渡期的兼容别名（--bg / --surface / --muted 等旧 token 名）已随五个
  // 遗留特性样式表的迁移整体删除。钉住这条：任何新代码都不得重新引入
  // 非 --color-/--font-/--space-/--radius-/--shadow- 体系的裸别名。
  it("declares no legacy token aliases", () => {
    ...
    expect(declared.filter((t) => legacy.includes(t))).toEqual([]);
  });
});
