...
- Round flow (all in `curation_service.go`):
  1. `curationMu` serializes rounds. Load catalog; `fingerprint := catalogFingerprint(catalog)`; `runID := stableID("curation-run", fingerprint)`; if `s.repository.CurationRun(ctx, runID)` exists AND its Status is not `failed` → return it (idempotent skip, zero curator calls). An all-failed round is re-run so a transient outage cannot poison its fingerprint.
  ...
  4. Judge each candidate with per-candidate isolation (an error in one candidate records a failed outcome, continues). Judge errors degrade immediately to a `keep`-shaped outcome with `Status: TargetStatusFailed` and the error string — LLM retries live solely in the adapter (Task 10), matching the planner/editor/indexer precedent. A page touched by the pair lane is excluded from the quality lane in the same round.
