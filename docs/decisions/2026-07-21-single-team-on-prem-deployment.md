...
### 2.1. on-prem Channel 使用 credential-bound Agent identity

Knowledge Capsule Channel 是部署 Module 的传输能力，不是 Team Note 或 LLM Wiki 的
写入入口。发送方身份完全取自 Agent Credential，接收方由安装实例内唯一的
`to_agent_id` 寻址。服务端保存 envelope 和 delivery 状态，但不自动把 Capsule 摄入
Team Memory。

Channel 使用独立的 `channel_send` 与 `channel_receive` 权限，并提供
`/v1/channel/envelopes` 下的 send、inbox/outbox list、get、accept 和 archive 接口。
第一版只支持定向 Agent 投递，不支持 Team 广播、Friend 或跨实例 federation。
