...
// SessionRepository atomically persists session streams and schedules their extraction.
type SessionRepository struct {
	pool       *pgxpool.Pool
	enqueuerMu sync.RWMutex
	enqueuer   ExtractionEnqueuer
}

func (r *SessionRepository) ConfigureExtractionEnqueuer(enqueuer ExtractionEnqueuer) error {
	if enqueuer == nil {
		return fmt.Errorf("configure extraction enqueuer: enqueuer is required")
	}
	r.enqueuerMu.Lock()
	defer r.enqueuerMu.Unlock()
	if r.enqueuer != nil {
...
func (r *SessionRepository) AppendSession(ctx context.Context, scopeID string, batch session.SessionBatch) (receipt session.IngestReceipt, returnedErr error) {
	...
	if enqueuer := r.extractionEnqueuer(); enqueuer != nil {
		jobID, enqueueErr := enqueuer.EnqueueTx(ctx, tx, scopeID, batch.Events[0].Actor, receipt.Cursor, batch.Complete)
		if enqueueErr != nil {
...
func (r *SessionRepository) extractionEnqueuer() ExtractionEnqueuer {
	r.enqueuerMu.RLock()
	defer r.enqueuerMu.RUnlock()
	return r.enqueuer
}

func (r *SessionRepository) SessionLatestSequence(ctx context.Context, scopeID string, actor session.Actor) (int64, error) {
...
func (r *SessionRepository) SessionEvents(ctx context.Context, scopeID string, actor session.Actor, after int64, limit int) ([]session.SessionEvent, error) {
...
func (r *SessionRepository) SessionEventsBefore(ctx context.Context, scopeID string, actor session.Actor, atOrBefore int64, limit int) ([]session.SessionEvent, error) {
...
func (r *SessionRepository) ExtractionCursor(ctx context.Context, scopeID string, actor session.Actor) (int64, error) {
	var cursor int64
	err := r.pool.QueryRow(ctx, `
...
func (r *SessionRepository) AdvanceExtractionCursor(ctx context.Context, scopeID string, actor session.Actor, cursor int64) (returnedErr error) {
	tx, err := r.pool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
