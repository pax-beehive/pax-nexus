...
func linkProtocolArtifacts(directory, version string, files map[string]string) (string, error) {
	...
	if version != "recall-v2" {
		return ArtifactSchemaVersion, nil
	}
	for key, path := range map[string]string{
		...
		if _, err := os.Stat(filepath.Join(directory, path)); err == nil {
			...
			return "", fmt.Errorf("inspect recall eval v2 artifact %q: %w", path, err)
		}
	}
	return ArtifactSchemaVersionRecallV2, nil
}
